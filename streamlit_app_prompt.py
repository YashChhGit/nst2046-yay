...
def json_to_structured_prompt(d):
    ...
    # Normalize keys and pull safely
    tempo = d.get("Tempo")
    ...
    if instruments:
        # make sure instruments are comma-separated cleanly
        instruments_str = instruments if ',' in instruments else instruments
        parts.append(f"Instruments: {instruments_str}")

    # Merge tone + dynamics into style-like info if style missing
    if not style:
        style_bits = []
        if tone: style_bits.append(tone)
        if dynamics: style_bits.append(dynamics)
        style = ", ".join(style_bits) if style_bits else None
    ...
    # Tempo section
    tempo_section = []
    if tempo: tempo_section.append(f"BPM: {int(round(tempo)) if isinstance(tempo,(int,float)) else tempo}")
    ...
    # Join all with " | " to match your samples
